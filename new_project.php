			...
			<form action="" id="manage-project">
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<select name="status" id="status" class="custom-select custom-select-sm">
								<option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>chờ phê
									duyệt
								</option>
								<option value="3" <?php echo isset($status) && $status == 3 ? 'selected' : '' ?>>trong quá
									trình
								</option>
								<option value="5" <?php echo isset($status) && $status == 5 ? 'selected' : '' ?>>hoàn
									thành
								</option>
							</select>
						</div>
				...
				<div class="row">
					<?php if ($_SESSION['login_type'] == 1): ?>
						<div class="col-md-6">
							<div class="form-group">
								<label for="" class="control-label">quản lí dự án</label>
								<select class="form-control form-control-sm select2" name="manager_id">
									...
									while ($row = $managers->fetch_assoc()):
										?>
										<option value="<?php echo $row['id'] ?>" <?php echo isset($manager_id) && $manager_id == $row['id'] ? "selected" : '' ?>><?php echo ucwords($row['name']) ?>
										</option>
									<?php endwhile; ?>
								</select>
							</div>
		...
		<div class="card-footer border-top border-info">
			<div class="d-flex w-100 justify-content-center align-items-center">
				...
				<button class="btn btn-flat bg-gradient-secondary mx-2" type="button"
					onclick="location.href='index.php?page=project_list'">huỷ</button>
			</div>
		</div>
	</div>
</div>
